...
export interface HostDevice {
    ...
    sendPitchBendToHost: (value: number) => void;
    sendModulationToHost: (value: number) => void;
    sendExpressionToHost: (value: number) => void;
    sendBreathControlToHost: (value: number) => void;
    sendSustainToHost: (value: number) => void;
    log: (message: string) => void;
    trimText: (text: string, maxLength: number, padding: boolean) => string;
}
...
class ControlHandler {
    static kInvalidHandlerIndex = -1;
    sendMidi: (status: number, data1: number, data2: number) => void;
    trimText: (text: string, maxLength: number, padding: boolean) => string;
    name: string;
    device: ControlSurfaceDevice | null;
    handlerIndex: number;
    ...
    constructor() {
         ...
         */
        this.trimText = function (text: string, maxLength: number, padding: boolean) {
     ...
     */
    sendValue(value: any, flags: number) { }
    /**
     ...
     * @param data2  MIDI message data byte 2
     */
    receiveMidi(status: number, data1: number, data2: number) {
        return false;
    }
    /**
     ...
     */
    sendSysex(sysexBuffer: SysexBuffer) {
...
class ControlSurfaceDevice {
    ...
    midiOutConnected: boolean;
    receiveHandlers: ControlHandler[];
    sysexSendBuffer: SysexBuffer;
    debugLog: boolean;
    constructor() {
    ...
    onMidiOutConnected(state: boolean) {
     ...
     * @param data2  MIDI message data byte 2.
     */
    onMidiEvent(status: number, data1: number, data2: number) {
     ...
     */
    addHandler(handler: ControlHandler) {
        handler.device = this;
        handler.handlerIndex = this.hostDevice?.addHandler(handler.name, handler) ?? -1;
    }
    /**
     * Register a receive control handler.
     * @param handler  Control handler to add.
     */
    addReceiveHandler(handler: ControlHandler) {
     ...
     */
    sendPitchBendToHost(value: number) {
     ...
     * @param value  Normalized parameter value.
     */
    sendModulationToHost(value: number) {
     ...
     * @param value  Normalized parameter value.
     */
    sendExpressionToHost(value: number) {
     ...
     * @param value  Normalized parameter value.
     */
    sendBreathControlToHost(value: number) {
     ...
     */
    sendSustainToHost(value: number) {
